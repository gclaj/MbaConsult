...
3. The school's most recent EMPLOYMENT REPORT — dig deep, this drives career-path analysis: % employed at graduation / 3 months, median base salary overall, the % of the class going into EACH industry (consulting, technology, financial services, PE/VC, healthcare, energy, etc.) with per-industry median salaries where published, the % by function, the top hiring employers by name, and any notable pipelines (e.g., specific consulting firms, veterans' recruiting channels, entrepreneurship-through-acquisition, sponsored programs).
...
FIRST, perform the CAREER PATH ANALYSIS (careerPathAnalysis) — this anchors everything else:
1. Enumerate the candidate's plausible post-MBA career paths. Derive them from the candidate's stated goals, the strategic-context theses (evaluate EACH live thesis as its own path), and any strong adjacent path this school's employment report obviously supports for someone with this background.
2. Cross-reference each path against the school's EMPLOYMENT REPORT: pipelineStrength (0–100) = how heavily this school actually places graduates into that path (use the industry/function percentages, named employers, and salary data as evidence — cite them in the evidence field); backgroundAlignment (0–100) = how credibly THIS candidate recruits for it given their background; overallViability (0–100) = the combined judgment.
3. Choose recommendedPath: the best career path for this candidate AT THIS SCHOOL, driven primarily by the employment report's revealed pipeline. Be honest — if the school barely places into a path the candidate wants, say so and recommend the strongest viable alternative framing.
The recommended path must then drive the rest of the profile: the Career Goals dimension scores against it, positioning presents it, and every essay strategy assumes it as the stated goal (adapted to reapplication constraints where the strategic context requires).

THEN score fit on a 0–100 scale across these dimensions: Academic Readiness, Professional Trajectory & Impact, Leadership Evidence, Community & Values Alignment, Career Goals ↔ School Pipeline Fit, and Uniqueness of Contribution to the Class. Be direct about gaps — a candidate is better served by honesty than flattery. For each essay, define a strategy and the specific TYPES of stories the candidate needs (e.g., "leading through resistance," "community impact tied to values," "moment that shaped career vision").`;
